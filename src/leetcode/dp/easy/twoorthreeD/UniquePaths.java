...
// corner (i.e., grid[m - 1][n - 1]). The robot can only move either down or right at any point
// in time.
//
...
public class UniquePaths {
    public int uniquePaths(int m, int n) {
        int[] row = new int[n];
        Arrays.fill(row, 1);
//Number of ways to reach each cell in the current row.
        for (int i = 1; i < m; i++) {
            for (int j = 1; j < n; j++) {
                // New value = current value (from top) + previous value (from left)
                row[j] = row[j] + row[j - 1];//Number of ways to reach each cell in the current row.
